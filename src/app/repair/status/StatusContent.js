...
export default function StatusContent() {
  ...
  const [currentStep, setCurrentStep] = useState(() => {
    if (paymentStatus === 'success') {
      if (
        currentStepParam === STEPS.ESTIMATION_APPROVAL ||
        currentStepParam === STEPS.REPAIR_IN_PROGRESS
      ) {
        return STEPS.REPAIR_IN_PROGRESS;
      } else if (
        currentStepParam === STEPS.DELIVERY_TO_USER ||
        currentStepParam === STEPS.COMPLETED
      ) {
        return STEPS.COMPLETED;
      }
    }
    return currentStepParam;
  });
  ...
  useEffect(() => {
    if (paymentStatus === 'success' && !toastShownRef.current) {
      ...
      if (
        ...
        showToast({
          type: 'success',
          message: 'Payment of ₹' + advanceAmount + ' received! Repair started.',
        });
      } else if (
        currentStepParam === STEPS.DELIVERY_TO_USER ||
        currentStepParam === STEPS.COMPLETED
  ...
  }, [paymentStatus, currentStepParam, showToast, advanceAmount]);
  ...
  useEffect(() => {
    ...
    if (currentStep === STEPS.DIAGNOSIS && timer >= 100) {
      nextStep = STEPS.ESTIMATION_APPROVAL;
    } else if (currentStep === STEPS.REPAIR_IN_PROGRESS && timer >= 100) {
      nextStep = STEPS.READY_FOR_DELIVERY;
    } else if (currentStep === STEPS.READY_FOR_DELIVERY && deliveryTimer <= 0) {
      nextStep = STEPS.DELIVERY_TO_USER;
    }
    if (nextStep) {
      const id = requestAnimationFrame(() => handleStepChange(nextStep));
      return () => cancelAnimationFrame(id);
    }
  }, [timer, deliveryTimer, currentStep, handleStepChange]);
  ...
  return (
    <main className={styles.main}>
            ...
            return (
              <div key={step.key} className={styles.timelineItem}>
                <div className={styles.timelineLeft}>
                  <div
                    className={`${styles.marker} ${isCurrent ? styles.markerActive : ''} ${isDone ? styles.markerCompleted : ''}`}
                  >
                    {isDone ? '✓' : step.icon}
                  </div>
                  {!isLast && (
                    <div
                      className={`${styles.timelineConnector} ${isDone ? styles.timelineConnectorActive : ''}`}
                    />
                  )}
                </div>
                ...
                <div className={styles.timelineContent}>
                  <h3
                    className={`${styles.stepTitle} ${isCurrent ? styles.stepTitleActive : ''} ${isDone ? styles.stepTitleCompleted : ''}`}
                  >
                    {step.title}
                  </h3>
                    ...
                    <div className={styles.statusCard}>
                      ...
                      {(step.key === STEPS.PICKUP_SCHEDULED ||
                        ...
                        <>
                          <p className={styles.statusDescription}>
                            {step.key === STEPS.PICKUP_SCHEDULED
                              ? 'Rider is arriving at your location.'
                              : 'Rider is heading to our service center.'}
                          </p>
                          ...
                          {step.key === STEPS.PICKUP_SCHEDULED && (
                            <Button
                              ...
                              onClick={() => handleStepChange(STEPS.RIDER_TO_CENTER)}
                          ...
                          {step.key === STEPS.RIDER_TO_CENTER && (
                            <Button
                              variant="primary"
                              className={styles.nextBtn}
                              onClick={() => handleStepChange(STEPS.DIAGNOSIS)}
                              style={{ marginTop: '1rem', width: '100%' }}
                            >
                      ...
                      {step.key === STEPS.DIAGNOSIS && (
                        <div className={styles.progressBox}>
                          <p className={styles.statusDescription}>
                            Our experts are diagnosing the issues. Please wait...
                          </p>
                        ...
                        <div className={styles.estimateCard}>
                          ...
                          <div className={styles.actionRow}>
                            <Button
                              variant="primary"
                              onClick={() =>
                                handlePaymentRedirect(advanceAmount, STEPS.REPAIR_IN_PROGRESS)
                              }
                              style={{ flex: 2 }}
                            >
                              Pay ₹{advanceAmount}
                            ...
                            <Button
                              ...
                              onClick={() =>
                                showToast({
                                  type: 'error',
                                  message: 'Order cancelled. Item will be returned.',
                                })
                              }
                      ...
                      {step.key === STEPS.REPAIR_IN_PROGRESS && (
                        <div className={styles.progressBox}>
                          <p className={styles.statusDescription}>
                            Technician is currently working on your device.
                          </p>
                          <div className={styles.progressBar}>
                            <div className={styles.progressFill} style={{ width: `${timer}%` }} />
                          </div>
                      ...
                      {step.key === STEPS.READY_FOR_DELIVERY && (
                        <div className={styles.deliveryChoice}>
                          <p className={styles.statusDescription}>
                            Repair successful! How would you like to receive your item?
                          </p>
                          ...
                          <div className={styles.actionRow}>
                            <Button
                              variant="primary"
                              onClick={() => handleStepChange(STEPS.DELIVERY_TO_USER)}
                              style={{ flex: 1 }}
                            >
                              Deliver to Me
                            </Button>
                            <Button
                              ...
                              onClick={() =>
                                showToast({
                                  type: 'info',
                                  message: 'Self-pickup chosen. Center open till 8pm.',
                                })
                              }
                              style={{ flex: 1, border: '1px solid var(--color-border)' }}
                            >
                      ...
                      {step.key === STEPS.DELIVERY_TO_USER && (
                        <>
                          <p className={styles.statusDescription}>
                            Rider is on the way with your repaired item.
                          </p>
                          <DriverInfo name="Raju Kumar" phone="+91 98765 43210" />
                          <TrackingMap
                            status="delivery"
                          ...
                          <div className={styles.receipt}>
                            ...
                            <Button
                              variant="primary"
                              onClick={() => handlePaymentRedirect(finalBalance, STEPS.COMPLETED)}
      ...
      {showRating && <RatingModal onClose={handleRatingSubmit} onSubmit={handleRatingSubmit} />}
